...
In parallel, we will maintain a `dev` branch in `christso/kata` that merges all layers so the feature can be used internally immediately.
...
- Maintain a `dev` branch that merges all the above layers (rebased as upstream PRs land).
- This branch can be used immediately for real work against multi-repo boards.
- Periodically rebase `dev` onto the latest upstream `main`.
...
- Gives the team immediate internal value via the `dev` branch.
- Clear, bisectable history.

## Risks & Mitigations

- **Rebase pain**: Mitigate by keeping stacks relatively shallow and rebasing frequently.
- **Upstream movement**: The `dev` branch in our fork will absorb this.
